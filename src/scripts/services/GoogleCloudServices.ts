...
export class GoogleCloudServices
{
    async getUserInfo(token: string): Promise<UserInfo> {
        var response = await fetch(`https://www.googleapis.com/oauth2/v1/userinfo?alt=json&access_token=${token}`);
    ...
    async getLicense(token: string): Promise<UserLiscence> {
        ...
        var response = await fetch(`https://www.googleapis.com/chromewebstore/v1.1/userlicenses/${chrome.runtime.id}`, init);
        return await response.json();
    }
