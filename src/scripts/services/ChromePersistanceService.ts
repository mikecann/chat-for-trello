...
export class ChromePersistanceService implements IPersistanceService {
    ...
    watch<T>(key: string, callback: (newValue: T) => void): WatchDisposer {
        const listener = (changes: { [key: string]: chrome.storage.StorageChange }, areaName: string) => {            
            ...
            this.logger.debug("ChromePersistanceService watch detected change", {changes, areaName});
        ...
        return () => {
            ...
            this.logger.debug(`ChromePersistanceService no longer watching for changes changes to key '${key}'`);
        }
    }
