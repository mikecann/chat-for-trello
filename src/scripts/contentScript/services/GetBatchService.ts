...
export class GetBatchService
{
    ...
    dispatchBatch(): void
    {
        ...
        var urls = this.outgoingBatch
            .map(b => b.url.replace("https://trello.com/1", ""))
            .join(',');

        var url = constructURL("https://trello.com/1/batch", { urls: urls });
        this.serviceHelpers.get<any>(url)
    ...
    private onBatchSuccess(result: any[])
    {
        ...
        {
            if (this.outgoingBatch == null)
                throw new Error("Outgoing batch cannot be null!");
                
            var success = result[i]["200"];
            if (success == null)
                this.outgoingBatch[i].reject(result[i]);
            else
                this.outgoingBatch[i].resolve(result[i]["200"]);
        }
