...
import {
    messageType as extensionMesssageType,
    LogMessage
} from "../helpers/ExtensionMessagingLogger";
...
export class LogMessagesFromExtensionModel {
    ...
    listenForNewMessages() {
        chrome.runtime.onMessage.addListener((request, sender) => {
            if (request.type != extensionMesssageType) return;
    ...
    @action
    listenForModelChangesInTheBackground(background: BackgroundPage) {
        for (var msg of background.logMessagesModel.messages) this.messages.push(msg);

        chrome.runtime.onMessage.addListener((request, sender) => {
            if (request.type != messageType) return;
