...
export class AuthModel {
    ...
    @computed get premiumMembershipPurchase(): GoogleIAPPurchase | undefined {
        return this.purchases.find(p => p.sku == "premiummembership");
    }

    @computed get isOnFreeTrial(): boolean {
        return this.premiumMembershipPurchase != undefined && this.daysRemainingOnFreeTrial > 0;
    }

    @computed get userHasPremiumAccess(): boolean {
        return this.premiumMembershipPurchase != undefined && this.premiumMembershipPurchase.state == "ACTIVE";
    }
    ...
    @computed get daysSinceLicenseIssued(): number {
        return this.premiumMembershipPurchase == undefined ? 0 : 
            Math.floor((Date.now() - parseInt(this.premiumMembershipPurchase.createdTime)) / 1000 / 60 / 60 / 24);
    }

    @computed get daysRemainingOnFreeTrial(): number {
        return this.premiumMembershipPurchase == undefined ? 0 : 
...
export class BackgroundAuthModel extends AuthModel {
    ...
    async attemptNonInteractiveAuthentication() {
        try {
            ...
            this.logger.warn("BackgroundAuthModel could not authenticate non-interractively, the user will have to auth manually", e);
            runInAction(() => this.isLoading = false);
        }
    }

    @action async authenticate(interactive = true) {

        try {
            if (this.isLoading)
                throw new Error("Cannot start auth more than once");

            this.logger.debug("AuthModel attempting to authenticate to get auth token from chrome..");

            this.isLoading = true;

            var token = await this.chromeService.getAuthToken({ interactive });
            this.logger.debug("AuthModel token loaded. Getting user info..", { token });
    ...
    @action async purchasePremiumMembership() {
        ...
        this.logger.debug("BackgroundAuthModel google purchase returned, refreshing purchases..", resp);
    ...
    private async runAsyncAndCatchErrors(func: () => Promise<any>) {
        try {
            ...
            if (error.stack != undefined)
                runInAction(() => this.error = error.stack + "");
            else
